...
class vel_manipulator:
	...
	def transform_callback(self, msg):
		...
		self.tf_broadcaster(br, coords[index], "temp2", "camera_link", constant_trans, constant_quat)
			...
			# Suggested ways may include using dual quaternions or transformation matrices from the Eigen library

			# Looking up transform and storing it in fixed variable
			# try:
			# 	print("We tried")
			# 	fixed_trans = tfBuffer.lookup_transform('camera_link','ARuco'+str(marker_transforms[index].fiducial_id), rospy.Time.now(), rospy.Duration(0.5))
